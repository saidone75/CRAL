...
(deftest update-node
  (let [ticket (get-in (auth/create-ticket "admin" "admin") [:body :entry])
        ...
        node-body-create (model/map->NodeCreateBody {:name (.toString (UUID/randomUUID)) :node-type "cm:content"})
        node-id (get-in (core/create-node ticket parent-id node-body-create) [:body :entry :id])
        new-name (.toString (UUID/randomUUID))]
    (core/update-node ticket node-id (model/map->NodeUpdateBody {:name new-name}))
...
(deftest get-node-content
  (let [ticket (get-in (auth/create-ticket "admin" "admin") [:body :entry])
        parent-id (:id (get-guest-home))
        node-body-create (model/map->NodeCreateBody {:name (str (.toString (UUID/randomUUID)) ".txt") :node-type "cm:content"})
        node-id (get-in (core/create-node ticket parent-id node-body-create) [:body :entry :id])
